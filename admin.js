...
function renderAdminSongsTable(songs) {
  ...
  tbody.innerHTML = songs.map(s => `
    <tr>
      ...
      <td>${esc(s.singer)}</td>
      <td><span style="background:rgba(124,58,237,0.12);color:#a78bfa;padding:3px 8px;border-radius:6px;font-size:11px;">${esc(s.genre || '—')}</span></td>
      <td>${s.year || '—'}</td>
...
function renderUsersTable(users) {
  ...
  tbody.innerHTML = users.map(u => {
    ...
    return `<tr>
      ...
      <td><span style="background:rgba(255,63,108,0.1);color:#ff3f6c;padding:3px 8px;border-radius:6px;font-size:12px;">❤️ ${likedArr.length}</span></td>
      <td><span style="background:rgba(124,58,237,0.1);color:#a78bfa;padding:3px 8px;border-radius:6px;font-size:12px;">📂 ${playlistArr.length}</span></td>
      <td><div class="actions-cell">
