...
async function startApp() {
  ...
  window.addEventListener('popstate', (e) => {
    ...
    // Close playlist detail if open (back from playlist detail → playlists grid)
...
function showEntityView(name, songs, label, icon, field) {
  ...
  if (covers.length >= 4 && field !== 'movieName') {
    // 2×2 collage for multi-movie views
    img.classList.add('hidden'); ph.style.display = 'none';
    if (!collage) { collage = document.createElement('div'); collage.className = 'album-collage'; coverWrap.appendChild(collage); }
    collage.innerHTML = covers.slice(0,4).map(c => `<img src="${c}" onerror="this.style.display='none'" />`).join('');
...
function renderSongsListItem(song) {
  return `<div class="song-list-item ${currentQueue[currentSongIndex]?._id === song._id ? 'playing' : ''}"
    onclick="playSongFromListDirect(${allSongs.findIndex(s=>s._id===song._id)})"
    oncontextmenu="openContextMenu(event,'${song._id}')"
    data-song-id="${song._id}">
    ${song.coverUrl ? `<img class="song-list-cover" src="${song.coverUrl}" alt="" onerror="this.outerHTML='<div class=song-list-cover-ph>🎵</div>'" />` : `<div class="song-list-cover-ph">🎵</div>`}
    <div class="song-list-meta">
