...
let _otpTimer   = null;
let _otpEnd     = null;
let _demoOtp    = null;   // OTP generated locally when server is offline
let _demoSignup = null;   // { name, email, password } for demo account creation

async function registerUser() { return sendSignupOtp(); }

// ── Banner helpers (shown only in demo/offline mode) ──
function _showDemoOtpBanner(otp) {
  const b = document.getElementById('demoOtpBanner');
  const c = document.getElementById('demoOtpCode');
  if (b && c) { c.textContent = otp; b.style.display = 'block'; }
}
function _hideDemoOtpBanner() {
  const b = document.getElementById('demoOtpBanner');
  if (b) b.style.display = 'none';
}

// ── Step 1: validate → wake server → send OTP to email ──
async function sendSignupOtp() {
  ...
  document.getElementById('otpSendText').style.display = 'none';
  document.getElementById('otpSendSpinner').classList.remove('hidden');

  // ── Wake Render server first (may be sleeping) ──
  const alive = await wakeUpServer('signupError', 28000);

  if (!alive) {
    // Server unreachable — demo mode: show OTP on screen
    _setDemoMode(name, email, password);
    document.getElementById('otpSendText').style.display = '';
    document.getElementById('otpSendSpinner').classList.add('hidden');
    return;
  }

  // ── Server alive — send real OTP email ──
  try {
    ...
    if (!res.ok) {
      showAuthError('signupError', data.message || 'Failed to send OTP. Try again.');
      return;
    }

    // ✅ OTP email sent — go to Step 2
    _demoOtp = null; _demoSignup = null;
    _hideDemoOtpBanner();
...
function _setDemoMode(name, email, password) {
  _demoOtp    = Math.floor(100000 + Math.random() * 900000).toString();
  _demoSignup = { name, email, password };
  document.getElementById('otpSentEmail').textContent = email;
  _showDemoOtpBanner(_demoOtp);
  _goToStep2();
}
...
function _goToStep2() {
  ...
  clearOtpBoxes();
  startOtpCountdown();
  setTimeout(() => document.querySelector('.otp-box')?.focus(), 120);
}
...
async function verifySignupOtp() {
  ...
  if (otp.length < 6)
    return showAuthError('signupError', 'Please enter the complete 6-digit OTP.');
  showAuthError('signupError', '');

  document.getElementById('otpVerifyText').style.display = 'none';
  ...
  if (_demoOtp !== null) {
    if (otp !== _demoOtp) {
      _shakeBoxes();
      showAuthError('signupError', '❌ Invalid OTP. Please check and try again.');
      document.getElementById('otpVerifyBtn').disabled = false;
      document.getElementById('otpVerifyText').style.display = '';
    ...
    if (users.find(u => u.email === d.email)) {
      showAuthError('signupError', 'Email already registered. Please login.');
      document.getElementById('otpVerifyBtn').disabled = false;
      document.getElementById('otpVerifyText').style.display = '';
      document.getElementById('otpVerifySpinner').classList.add('hidden');
      return;
    ...
    users.push({ id: uid, name: d.name, email: d.email, password: d.password, createdAt: new Date().toISOString() });
    localStorage.setItem('rhythmUsers', JSON.stringify(users));
    currentUser = { id: uid, name: d.name, email: d.email };
    localStorage.setItem('rhythmUser', JSON.stringify(currentUser));
    _demoOtp = null; _demoSignup = null;
    document.getElementById('otpVerifyText').style.display = '';
    document.getElementById('otpVerifySpinner').classList.add('hidden');
  ...
  try {
    ...
    if (!res.ok) {
      _shakeBoxes();
      showAuthError('signupError', data.message || '❌ Invalid OTP. Please check and try again.');
      document.getElementById('otpVerifyBtn').disabled = false;
      return;
    }
...
async function resendSignupOtp() {
  ...
  const msgEl  = document.getElementById('otpResendMsg');
  ...
  try {
    ...
    const data = await res.json();
    if (res.ok) {
      _demoOtp = null; _hideDemoOtpBanner();
      msgEl.textContent = '✅ New code sent to ' + email + '!';
      msgEl.style.color = '#4ade80';
      clearOtpBoxes(); startOtpCountdown();
      setTimeout(() => document.querySelector('.otp-box')?.focus(), 120);
    } else {
      msgEl.textContent = data.message || 'Failed to resend.';
      msgEl.style.color = 'var(--accent)';
    }
  } catch {
    // Demo fallback for resend
    _demoOtp    = Math.floor(100000 + Math.random() * 900000).toString();
    _demoSignup = { name, email, password: pwd };
    _showDemoOtpBanner(_demoOtp);
    msgEl.textContent = '⚠️ Server offline — new OTP shown above.';
...
function setOtpStep(n) {
  [1, 2, 3].forEach(i => {
    const step = document.getElementById('otpStep' + i);
    const line = document.getElementById('otpLine'  + i);
    if (!step) return;
...
function startOtpCountdown() {
  clearInterval(_otpTimer);
  _otpEnd = Date.now() + 10 * 60 * 1000;
  updateOtpCountdown();
  _otpTimer = setInterval(updateOtpCountdown, 1000);
}
function updateOtpCountdown() {
  const el  = document.getElementById('otpCountdown');
  const row = document.getElementById('otpTimer');
  if (!el) return;
  const rem  = Math.max(0, _otpEnd - Date.now());
  const mins = Math.floor(rem / 60000);
  const secs = Math.floor((rem % 60000) / 1000);
  el.textContent = `${mins}:${secs.toString().padStart(2, '0')}`;
  row.className  = 'otp-timer';
  if (rem < 60000)       row.classList.add('danger');
  else if (rem < 180000) row.classList.add('warning');
  if (rem === 0) {
    clearInterval(_otpTimer);
    showAuthError('signupError', 'Code expired. Please request a new one.');
    document.getElementById('otpVerifyBtn').disabled = true;
  }
}
...
function clearOtpBoxes() {
  ...
  document.getElementById('otpVerifyBtn').disabled = true;
  document.getElementById('otpResendMsg').textContent = '';
}
...
function otpBoxInput(el, idx) {
  ...
  const all = [...document.querySelectorAll('.otp-box')].every(b => b.value);
  document.getElementById('otpVerifyBtn').disabled = !all;
}

// ── Backspace → go to previous box ──
function otpBoxKey(e, el, idx) {
...
function showForgotPassword() {
  ...
  document.getElementById('fpStepInbox').classList.add('hidden');
  setFpStep(1);
...
async function sendForgotEmail() {
  ...
  document.getElementById('fpSendText').style.display    = 'none';
  document.getElementById('fpSendSpinner').classList.remove('hidden');
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok) { errEl.textContent = data.message || 'Something went wrong.'; return; }
    _fpEmailSent = email;
    document.getElementById('fpSentTo').textContent = email;
    document.getElementById('fpStepEmail').classList.add('hidden');
    document.getElementById('fpStepInbox').classList.remove('hidden');
    setFpStep(2);
  } catch {
    errEl.textContent = 'Cannot connect to server. Try again.';
  } finally {
    document.getElementById('fpSendText').style.display    = '';
    document.getElementById('fpSendSpinner').classList.add('hidden');
  }
}

async function resendForgotEmail() {
  const msgEl = document.getElementById('fpResendMsg');
  ...
  try {
    const res  = await fetch(`${API_BASE_URL}/auth/forgot-password`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: _fpEmailSent })
    });
    const data = await res.json();
    if (res.ok) { msgEl.textContent = '✅ Email resent!'; msgEl.style.color = '#4ade80'; }
    else        { msgEl.textContent = data.message || 'Failed to resend.'; }
