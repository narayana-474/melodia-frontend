  ...
  el.innerHTML = songs.map((song, i) => `
    <div class="song-list-item ${currentQueue[currentSongIndex]?._id === song._id ? 'playing' : ''}"
      ...
      <div class="song-list-meta">
        ...
        <div class="song-list-artist">${esc(song.singer || song.musicDirector || '—')} • ${esc(song.movieName)}</div>
      </div>
...
// ── WAKE LOCK — keeps page alive so audio continues when screen locks ──
let _wakeLock = null;
async function acquireWakeLock() {
  ...
  } catch(e) { /* silently ignore — MediaSession keeps lock-screen controls working */ }
}
async function releaseWakeLock() {
  try {
    if (_wakeLock && !_wakeLock.released) { await _wakeLock.release(); _wakeLock = null; }
  } catch(e) {}
}
// Re-acquire wake lock when user returns to page (e.g. unlocks phone mid-song)
...
audio.addEventListener('ended', () => {
  if (loopMode === 'single') { audio.currentTime = 0; audio.play(); acquireWakeLock(); return; }
  if (isShuffle) { currentSongIndex = Math.floor(Math.random() * currentQueue.length); loadAndPlay(); return; }
  if (currentSongIndex < currentQueue.length - 1) {
    currentSongIndex++; loadAndPlay();
  } else if (loopMode === 'playlist') {
...
// ===================================================================
// ARTIST ALIAS MAP — maps nicknames/abbreviations → canonical full name
// Add more entries here as needed
// ===================================================================
const ARTIST_ALIASES = {
  // Devi Sri Prasad
  'dsp':                 'Devi Sri Prasad',
  'd.s.p':               'Devi Sri Prasad',
  'devi sriprasad':      'Devi Sri Prasad',
  ...
  'mahesh':              'Mahesh Babu',
  'prince':              'Mahesh Babu',
  'superstar mahesh':    'Mahesh Babu',
  'mb':                  'Mahesh Babu',

  // Vijay Deverakonda
  'vd':                  'Vijay Deverakonda',
...
/**
 * Resolves aliases: returns array of candidate query strings.
 * e.g. "dsp" → ["dsp", "devi sri prasad"]
 */
function resolveAliases(query) {
  const q = query.toLowerCase().trim();
  const candidates = new Set([q]);
  if (ARTIST_ALIASES[q]) candidates.add(ARTIST_ALIASES[q].toLowerCase());
  for (const [alias, canonical] of Object.entries(ARTIST_ALIASES)) {
    if (q === alias || q.startsWith(alias + ' ') || q.endsWith(' ' + alias) || q.includes(' ' + alias + ' ')) {
      candidates.add(canonical.toLowerCase());
    }
  }
  return [...candidates];
}
...
function liveSearch(query) {
  ...
  // Resolve aliases — e.g. "dsp" → ["dsp", "devi sri prasad"]
  const queryCandidates = resolveAliases(coreQuery);

  // Helper: does any candidate match a string?
  function anyMatch(str) {
    if (!str) return false;
    const s = str.toLowerCase();
    return queryCandidates.some(q => s.includes(q) || q.includes(s));
  }

  // Broad match across all fields using all alias candidates
  const broadResults = allSongs.filter(s => {
    ...
    return fields.some(f => anyMatch(f || ''));
  });

  if (!broadResults.length) { showNoResults(raw); return; }

  // ── Step 1: Movie album match ───────────────────────────────────
  {
    const movieVals = new Set();
    broadResults.forEach(s => { if (s.movieName) movieVals.add(s.movieName); });
    let bestVal = null, bestCount = 0;
    for (const val of movieVals) {
      if (!anyMatch(val)) continue;
      const count = allSongs.filter(s => fuzzyMatch(s.movieName || '', val)).length;
      if (count > bestCount) { bestCount = count; bestVal = val; }
    }
    if (bestVal) {
      const matched = allSongs.filter(s => fuzzyMatch(s.movieName || '', bestVal));
      showEntityView(bestVal, matched, 'Movie Album', '🎬', 'movieName');
      return;
    }
  }

  // ── Step 2: Artist search — scan ALL fields, merge songs per artist ──
  const ARTIST_FIELDS = ['singer', 'musicDirector', 'cast', 'movieDirector', 'genre'];

  // artistMap: display name → Map<songId, song>  (Map deduplicates songs)
  const artistMap = {};

  allSongs.forEach(s => {
    ARTIST_FIELDS.forEach(field => {
      const raw2 = s[field] || '';
      const parts = (field === 'cast' || field === 'singer')
        ? raw2.split(',').map(p => p.trim()).filter(Boolean)
        : raw2 ? [raw2.trim()] : [];

      parts.forEach(name => {
        const nName = name.toLowerCase();
        // Match against ALL alias candidates
        const matches = queryCandidates.some(q => nName.includes(q) || q.includes(nName));
        if (!matches) return;
        if (!artistMap[name]) artistMap[name] = new Map();
        artistMap[name].set(s._id, s);
      });
    });
  });

  // Score and pick best artist
  let bestArtist = null, bestSongs = [], bestScore = -1;
  for (const [name, songMap] of Object.entries(artistMap)) {
    const n = name.toLowerCase();
    let score = songMap.size;
    const isExact     = n === coreQuery;
    const isCanonical = queryCandidates.some(q => n === q);
    if (isExact)          score += 10000;
    else if (isCanonical) score += 5000;
    else if (queryCandidates.some(q => n.includes(q))) score += 1000;
    else if (queryCandidates.some(q => q.includes(n))) score += 100;
    if (score > bestScore) {
      bestScore = score; bestArtist = name; bestSongs = [...songMap.values()];
    }
  }

  if (bestArtist) {
    showEntityView(bestArtist, bestSongs, 'Artist', '🎤', 'artist');
    return;
  }
...
function showEntityView(name, songs, label, icon, field) {
  ...
  if (field === 'movieName') {
    ...
    if (movies.length > 0) sub += `  •  ${movies.length} movie${movies.length !== 1 ? 's' : ''}`;
  }
  ...
  const collage = coverWrap.querySelector('.album-collage');
  ...
  if (field === 'movieName') {
    ...
    // Artist view: neutral person SVG — suits singers, actors, directors, lyricists
    img.classList.add('hidden'); ph.style.display = 'flex';
    ph.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="54" height="54" viewBox="0 0 24 24" fill="rgba(255,255,255,0.75)">
      <path d="M12 12c2.76 0 5-2.24 5-5s-2.24-5-5-5-5 2.24-5 5 2.24 5 5 5zm0 2c-3.33 0-10 1.67-10 5v2h20v-2c0-3.33-6.67-5-10-5z"/>
    </svg>`;
  }

  // Artist view: always group by movie for clarity
  if (field !== 'movieName') {
