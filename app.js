...
// ── Wake Render server (free tier sleeps after 15 min inactivity) ──
async function wakeUpServer(statusElId, maxWaitMs = 28000) {
  ...
  if (el) { el.style.color = '#f59e0b'; el.textContent = '⏳ Waking up server…'; }
...
// Keep registerUser as alias so any existing call still works
...
async function sendSignupOtp() {
  ...
  try {
    ...
    showAuthError('signupError', 'Cannot connect to server. Please try again.');
  } finally {
...
async function verifySignupOtp() {
  ...
  if (otp.length < 6)
    return showAuthError('signupError', 'Please enter the complete 6-digit code.');
  ...
  try {
    ...
    if (!res.ok) {
      ...
      showAuthError('signupError', data.message || 'Invalid code. Try again.');
...
async function sendForgotEmail() {
  ...
  const btnText    = document.getElementById('fpSendText');
  const btnSpinner = document.getElementById('fpSendSpinner');

  errEl.textContent = ''; errEl.style.color = 'var(--accent)';
  ...
  // Helper — always resets the button
  const resetBtn = () => {
  ...
  const alive = await wakeUpServer(null, 28000);
  errEl.textContent = '';

  if (!alive) {
    errEl.style.color = 'var(--accent)';
    errEl.textContent = '❌ Server unreachable. Please try again in a moment.';
    resetBtn(); return;
  }
  ...
  try {
    ...
    // ✅ Reset link sent — show inbox panel
    _fpEmailSent = email;
...
async function resendForgotEmail() {
  ...
  btn.disabled = true; btn.textContent = 'Sending...';
  msgEl.textContent = ''; msgEl.style.color = 'var(--accent)';
  try {
    const res  = await fetch(`${API_BASE_URL}/auth/forgot-password`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
    ...
    const data = await res.json();
    if (res.ok) {
      msgEl.textContent = '✅ Reset link resent to ' + _fpEmailSent + '!';
      msgEl.style.color = '#4ade80';
    } else {
      msgEl.textContent = data.message || 'Failed to resend. Try again.';
    }
  } catch {
