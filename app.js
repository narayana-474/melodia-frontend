...
// Block native browser pull-to-refresh on the main page content.
// Only fires when scrolled to top AND dragging down on the page body (not on scrollable panels).
let _touchStartY = 0;
...
document.addEventListener('touchmove', (e) => {
  ...
  // Allow normal scroll inside fullscreen player and queue panel
  ...
  // Allow scroll inside any overflow-y:auto containers (lyrics, playlists, etc.)
  let el = e.target;
  ...
  // At top of page pulling down — block pull-to-refresh
...
function openNowPlayingScreen() {
  ...
  fs._swipeMoveHandler = (e) => {
    ...
    if (dy > 0 && fs.scrollTop <= 0) {
      e.preventDefault();
      fs.style.transform = `translateY(${Math.min(dy * 0.6, 200)}px)`;
      fs.style.opacity = String(Math.max(0.4, 1 - dy / 400));
    }
  };
  fs._swipeEndHandler = (e) => {
    isDragging = false;
    const dy = e.changedTouches[0].clientY - startY;
    if (dy > 80 && fs.scrollTop <= 0) {
      fs.style.transform = '';
      fs.style.opacity = '';
      closeNowPlayingScreen();
    } else {
      // Snap back
      fs.style.transition = 'transform 0.25s cubic-bezier(0.4,0,0.2,1), opacity 0.25s ease';
      fs.style.transform = '';
      fs.style.opacity = '';
      setTimeout(() => { fs.style.transition = ''; }, 260);
    }
  };
  fs.addEventListener('touchstart', fs._swipeHandler, { passive: true });
  fs.addEventListener('touchmove', fs._swipeMoveHandler, { passive: false });
